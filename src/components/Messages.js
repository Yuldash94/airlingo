...
export default function Messages( {user, token, greeting, setGreeting, topics, setTopics, loadTopics, topicId, setTopicId, userPhoto}) {
    ...
    const urlForAudio = `${baseUrl}/api/audio/`
    const urlOptions = {
        method: 'GET',
        headers: {
    ...
    async function loadMessages(topic) {
        setLoading(true)
        let response = await fetch(`${url}${topic}/messages`, urlOptions)
        if (!response.ok) {
        ...
        // console.log('last message: ', json.messages[json.messages.length-1].audioId)
        loadAudio(json.messages[json.messages.length-1].audioId)
